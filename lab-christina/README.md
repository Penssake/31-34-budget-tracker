...
* The Expense reducer supports a 'CATEGORY_CREATE' action, a 'CATEGORY_UPDATE' action, an 'EXPENSE_CREATE' action, an 'EXPENSE_UPDATE' action, an 'EXPENSE_REMOVE' action, and an 'EXPENSE_UPDATE_CATEGORY_ID action.
...
* EXPENSE_UPDATE maps the current state and returns an object with the new expense itemID replacing the one selected for update
* EXPENSE_REMOVE removes an expense by filtering and returning the current state of the app minus the ID that matches the one selected for removal.
* EXPENSE_UPDATE_CATEGORY_ID
```
